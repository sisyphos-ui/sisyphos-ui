...
const LoadingSpinner = ({ position }: { position: "start" | "center" | "end" }) => (
  <span className={`${CN.loadingSpinner} ${CN.loadingSpinner}--${position}`} aria-hidden="true">
...
const ChevronIcon = ({ open }: { open: boolean }) => (
  <span className={`${CN.iconDropdown} ${open ? "open" : ""}`} aria-hidden="true">
        ...
        {isDropdownOpen && dropdownItems && (
          <ul className={`${CN.dropdown} ${dropdownPosition}`} role="menu">
            {dropdownItems.map((item, index) => (
              <DropdownItem key={item.key ?? index} item={item} onSelect={closeDropdown} />
            ))}
          </ul>
        )}
      </div>
...
function DropdownItem({ item, onSelect }: { item: ButtonDropdownItem; onSelect: () => void }) {
