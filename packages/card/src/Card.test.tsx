...
describe("Card", () => {
  ...
  it("interactive adds role=button + tabIndex 0", () => {
    render(<Card interactive><Card.Body>x</Card.Body></Card>);
    expect(screen.getByRole("button")).toHaveAttribute("tabIndex", "0");
  });

  it("non-interactive has no role=button", () => {
    const { container } = render(<Card><Card.Body>x</Card.Body></Card>);
    expect(container.querySelector('[role="button"]')).toBeNull();
  });

  it("interactive fires onClick", async () => {
    const onClick = vi.fn();
    render(<Card interactive onClick={onClick}><Card.Body>x</Card.Body></Card>);
