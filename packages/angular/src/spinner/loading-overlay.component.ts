...
export class LoadingOverlay {
  ...
  @Input("variant") set variantInput(v: LoadingOverlayVariant) { this._variant.set(v); }
  @Input("open") set openInput(v: boolean) { this._open.set(v); }
  @Input("text") set textInput(v: string | undefined) { this._text.set(v); }
  @Input("blur") set blurInput(v: boolean) { this._blur.set(v); }
  @Input("spinnerColor") set spinnerColorInput(v: SpinnerColor) { this._spinnerColor.set(v); }
  @Input("spinnerSize") set spinnerSizeInput(v: SpinnerSize) { this._spinnerSize.set(v); }

  readonly rootClasses = computed(() =>
    [
      "sisyphos-loading-overlay",
      this._variant(),
      this._blur() && "blur",
    ]
      .filter(Boolean)
