...
@Component({
  ...
  styles: [
    `
      .sisyphos-alert-actions:empty { display: none; }
    `,
  ],
})
export class Alert implements OnDestroy {
  ...
  @Input("variant") set variantInput(v: AlertVariant) { this._variant.set(v); }
  @Input("color") set colorInput(v: AlertColor) { this._color.set(v); }
  ...
  @Input("hideIcon") set hideIconInput(v: boolean) { this._hideIcon.set(v); }
  @Input("closable") set closableInput(v: boolean) { this._closable.set(v); }
  @Input("closeAriaLabel") set closeAriaLabelInput(v: string) { this._closeAriaLabel.set(v); }
  @Input("autoCloseDuration") set autoCloseDurationInput(v: number | undefined) {
    this._autoCloseDuration.set(v);
  }
  @Input("role") set roleInput(v: string | undefined) { this._role.set(v); }
  @Input("hasCustomIcon") set hasCustomIconInput(v: boolean) { this.hasCustomIcon = v; }

  /** Emitted when the close button is clicked or autoCloseDuration elapses. */
  @Output() readonly close = new EventEmitter<void>();

  readonly rootClasses = computed(() =>
    `sisyphos-alert ${this._variant()} ${this._color()}`
  );
